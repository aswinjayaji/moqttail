...
use crate::flight::flight3::Flight3;
//use crate::flight::flight5::Flight5;
...
impl DTLSConn {
    ...
    async fn send(&mut self) -> Result<HandshakeState> {
        ...
        if let Some(pkts) = self.flights.clone() {
            // Need to check current_flight because this function is used for flights 1, 3, and 5
            if self.current_flight.to_string() == "Flight 1" && self.state.dtls_cli.flight1 {
                if self.state.dtls_cli.times1.unwrap() == 0 {
                    loop {
                        self.write_packets(pkts.clone()).await?;
                    }
                } else {
                    for _x in 0..self.state.dtls_cli.times1.unwrap() {
                        self.write_packets(pkts.clone()).await?;
                    }
                    println!(
                        "Sent {} Flight 1's to {}",
                        self.state.dtls_cli.times1.unwrap(),
                        self.state.dtls_cli.target
                    );
                }
            } else if self.current_flight.to_string() == "Flight 3" && self.state.dtls_cli.flight3 {
                if self.state.dtls_cli.times3.unwrap() == 0 {
                    loop {
                        self.write_packets(pkts.clone()).await?;
                    }
                } else {
                    for _x in 0..self.state.dtls_cli.times3.unwrap() {
                        self.write_packets(pkts.clone()).await?;
                    }
                    println!(
                        "Sent {} Flight 3's to {}",
                        self.state.dtls_cli.times3.unwrap(),
                        self.state.dtls_cli.target
                    );
                }
            } else if self.current_flight.to_string() == "Flight 5" && self.state.dtls_cli.flight5 {
                if self.state.dtls_cli.times5.unwrap() == 0 {
                    loop {
                        self.write_packets(pkts.clone()).await?;
                    }
                } else {
                    for _x in 0..self.state.dtls_cli.times5.unwrap() {
                        self.write_packets(pkts.clone()).await?;
                    }
                    println!(
                        "Sent {} Flight 5's to {}",
                        self.state.dtls_cli.times5.unwrap(),
                        self.state.dtls_cli.target
                    );
                }
            }
    ...
    async fn wait(&mut self) -> Result<HandshakeState> {
        ...
        if self.current_flight.to_string() == "Flight 1" {
            if !self.state.dtls_cli.flight1 && self.state.dtls_cli.flight3 {
                self.current_flight = Box::new(Flight3 {});
                return Ok(HandshakeState::Preparing);
            }
            if self.state.dtls_cli.flight1 && !self.state.dtls_cli.flight3 {
                return Err(Error::DtlsCliDone.into());
            }
        }
        if self.current_flight.to_string() == "Flight 3"
            && self.state.dtls_cli.flight3
            && !self.state.dtls_cli.flight5
        {
            return Err(Error::DtlsCliDone.into());
        }
