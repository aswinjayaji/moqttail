...
use exofense_dtls::cipher_suite::CipherSuiteId;
use exofense_dtls::config::ExtendedMasterSecretType;
use exofense_dtls::udp_conn::*;
use exofense_dtls::{config::Config, listener::listen};
use std::io::Write;
use std::sync::Arc;
...
async fn main() -> Result<()> {
    ...
    let listener = Arc::new(listen(host, cfg, None).await?);
    ...
    tokio::spawn(async move {
        while let Ok((dtls_conn, _remote_addr, _)) = listener2.accept().await {
