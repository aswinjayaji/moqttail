...
type SessionIdTypeConst = u8;
const   SESSION_ID_TYPE_NORNAL:SessionIdTypeConst       = 0b_0_00_0_0_0_00;
const   SESSION_ID_TYPE_PRE_DEFINED:SessionIdTypeConst  = 0b_0_00_0_0_0_01;
const   SESSION_ID_TYPE_SHORT:SessionIdTypeConst        = 0b_0_00_0_0_0_10;
const   SESSION_ID_TYPE_UNDEFINED:SessionIdTypeConst    = 0b_0_00_0_0_0_11;
...
impl ClientStruct {
    ...
    pub fn state_transition(&self, state_machine: &StateMachine, msg_type: u8)
        -> Option<(u8)> {
            ...
            self.state.compare_exchange(cur_state, result,
                                        Ordering::Acquire,
                                        Ordering::Relaxed);
            Some(result)
        }
...
impl Flags {
    ...
    #[inline(always)]
    pub fn sess_id_type(&self, input: u8) -> SessionIdTypeConst {
        input & 0b11
    }
    #[inline(always)]
    pub fn set(&self,
               ...
               sess_id_type: SessionIdTypeConst) -> u8 {
        dup | qos | retain | will | clean_session | sess_id_type
    }
}
...
fn main(){
            ...
            match socket.recv_from(&mut buf) {
                Ok ((size, addr)) => {
                    ...
                    if msg_type == MsgType::PUBLISH as u8 {
                        match verify_publish2(&buf, size) {
    ...
    let publish_thread = thread::spawn(move || {
        ...
        loop {
            ...
            let bytes = publish(1, msg_id, sub_msg, 1);

            publish_socket.send_to(&bytes, remote_addr);
            // schedule for timeout, check for QoS flags
/*
            let data = RetransmitData {
                bytes,
            };
